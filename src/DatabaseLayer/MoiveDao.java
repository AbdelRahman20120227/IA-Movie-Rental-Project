...
public class MoiveDao {
	...
	public static void  updateMovie(Movie movie) {
		try {
			Connection connection = DataSource.getInstance().getConnection() ;
			PreparedStatement preparedStatement = connection.prepareStatement("update movie set moviename=? , movieyear = ?  ,  movierating = ?  , movietype = ? , moviedescription = ? , quality = ? ,  quantity = ? , price = ?  , moviecast = ? where movieid = ?"); 
			preparedStatement.setString(1, movie.getMovieName());
			preparedStatement.setString(2, movie.getMovieYear());
			preparedStatement.setFloat(3, movie.getMovieReating());
			preparedStatement.setString(4, movie.getMovieType());
